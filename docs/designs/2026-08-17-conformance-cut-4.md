...
- **R10** and **T5** — the **acquisition** operation. R10's remainder is
  *"the acquisition path records dataset provenance instead"*, and T5's
  ...
  stays vacuous besides, a durable corpus minting records and not byte
  observations.
...
- **M5** — the **supersede family's adapter**. The founding case is one
  ...
  assessment still bound and a `supersedes` link (cut 3 `:280`). This cut
...
| domain boundary | D1, D2, D4, D5, D8, D9, D10 | 7 | **facets, manifests, and the registry compile**, unchanged. D1's check is a static reading of the `nodes` tree and gains no subject from durability — a static reading is not one of §2's four in-scope items, and the row's negative is a compile-time refusal; D5's projection needs the **corpus manifest** and identities to pin; D2's facet addition and D10's promotion trigger are edit-shaped besides, and wait on the **supersede family's adapter** with **retraction records** |

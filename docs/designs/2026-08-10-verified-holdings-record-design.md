...
completeness** — by replaying the registered surface against the carried
chain, the log's own replay run as an admission gate: a copy whose disk
surface validates enters service **read-only**; one that does not stays
unresolvable, and every dereference of it is an inconclusive attempt (§3),
never an `absent`. The cooperative steps still carry a
...
metadata-less root not yet admitted by §2's restore-act replay is
...
7. **The engine.** Persistence, the tamper-evident log's strengthening, and
   ...
   **read-only and unresolvable for holdings reads**; the restore command
   admitting a copy to read-only service only after replaying the
   registered surface against the carried chain, never granting

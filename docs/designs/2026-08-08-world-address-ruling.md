...
| packaging **X9, X10, X12** | the three receipt rows extend to the coreference reduction: X9's identical-states arm covers four receipts; X10's outcome arms gain the `indeterminate`-and-refuse consequence on **every** non-`validated` outcome, **`malformed` included** — the one that would otherwise have no consequence anywhere; and X12 gains both an omitted-attestation arm and a **wrong-balance** arm, so the reduction is checked and not only its membership |

...
**The interim writer is best-effort, and says so.** `atoms` A6–A8 do not exist,
and the substrate design's ruling stands: no crash-safe multi-file durability
before they land. Until then the writer is plain create-then-pointer-replace —
...
Every durability and crash-atomicity claim in §10 is **gated on A6–A8** and
...
doctrine. Rows marked **[A6–A8]** are gated: their mutation tests run against the
...
2. **Interim publication is neither crash-atomic nor durably pointed.** Until
   `atoms` A6–A8, the writer is best-effort create-then-pointer-replace; after a

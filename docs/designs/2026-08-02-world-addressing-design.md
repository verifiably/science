...
| one `uid`, **same** canonical address, two corpora | a **duplicate-location** migration state (§5), resolved by **`consolidate`** — *the operation was merge, retired 2026-08-08 (`2026-08-08-world-address-ruling.md` §5.4)* |
...
**`consolidate` selects a `uid`; it never mints one.** The survivor keeps one of
the input `uid`s, a distinct removed `uid` ceases to be live, and **no third `uid`
is created**. *(Restated 2026-08-08, `2026-08-08-world-address-ruling.md` §5.4; the operation was
merge, and this arm is preserved unchanged under **W16**.)*
...
> **The world index.** Science holds **three maps** (four from 2026-08-03, when
> the retraction map arrived with the correction-lifecycle design, 5a §4; **three
> again since 2026-08-08**, when the alias map retired with the stored alias —
> world address ruling §4.3), not one — and they do not share an identity:
>
...
> | ~~**alias**~~ | ~~alias → `[canonical address]`~~ | **retired 2026-08-08** — labels are rendered on read, never stored, so no corpus content derives this map. Three maps since (world address ruling §4.3) |
...
>
> All three are **derived** from corpus state and none is authoritative for it. The
...
The address, producers and retraction maps are **derived** from the corpora — the
...
> input** named by kernel §5.1's digest. The address and retraction maps are not in
> it *(nor was the alias map, which retired 2026-08-08)*.
>
...
|---|---|---|
| **duplicate location** | one identity, two corpora | the §4.3 migration state: reported, resolved by ~~an authored merge~~ **`consolidate`** *(2026-08-08 — merge retired; `consolidate` repairs storage and asserts nothing about identity, so no address retires and no inbound reference is rewritten)*, refused at the write boundary thereafter |
...
identity, covered-corpus identities included, while the address map, the retraction map
and the derivation receipt all stay out of it so that moving a file remains
epistemically silent (W5) *(the alias map was in this list until it retired
2026-08-08)*. An unpublished snapshot does not merely cost a resolution state; it leaves
...
> text: the **alias facet** and its schema; the world index's **alias map**,
> reducing four derived maps to three; the **`ambiguous`** resolution state on
> the reference path; and **structural merge** — the redirect-set, inbound
> rewrite, absent-referrer and `uid`-selection machinery of §4.3, of which only
> `consolidate`'s much smaller storage repair survives.
>
> Against that, the ruling **adds** an eleventh kind, a derived balance, a
> query-layer closure, and a pinned-authority renderer whose snapshot artifact is
> itself undesigned (§10 there, limitation 5). The net is very likely a
> reduction, and it is deliberately not quantified: this design's own warning
> against presenting a rebuild as a reduction applies to its amendments too.

...
> digest, under its own domain kind, of the `corpus_id` together with the sorted
> `(uid, node content identity)` pairs of every node in the corpus.
...
9. **The corpus manifest is a second new artifact, and adopting it is a minting
   ...
   **coordinated** re-minting — manifest plus a raw-forged admission for the new
   ...
   log, and any further field the manifest grows is outside every check in this
   design.
...
- ~~**Where the corpus manifest lives, and what else is in it.**~~ **CLOSED** by
  ...
  and optional fork provenance — nothing else, deliberately.
